...
const OfflineDevices = ({ propertyId }: any) => {
  const { isLargeScreen } = useSelector((state: RootState) => state.screenSize);

  const [data, setData] = useState([]);
  async function getData() {
    const response = await axios.get(
      `http://localhost:8080/device/offline/property/${propertyId}`
    );
  ...
  return (
    <>
      <div
        data-testid="offline-devices-container"
        className={isLargeScreen ? "m-4" : ""}
      >
        {isLargeScreen && (
