...
interface InsuranceData {
  ...
  eviction: number;
  unregisteredVehicle: number;
  vacantAlert: number;
  curfewActivity: number;
}

const Insurance: React.FC<InsuranceProps> = ({ propertyId }) => {
            ...
            <>
              <tr key={`${index}-average-occupancy`} className="bg-[#EDF1F7]">
                <td
                  ...
                  className="rounded-l-lg xl:px-6 lg:px-6 sm:px-2 xs:px-2 py-4 whitespace-nowrap text-sm font-medium text-[#5C626E] font-inter"
                >
                  Average Occupancy
                </td>
                <td
                  style={{ lineHeight: "3px" }}
                  className="xl:px-6 lg:px-6 sm:px-2 xs:px-2 py-4 whitespace-nowrap text-sm font-medium text-[#01337C] font-inter text-end rounded-r-lg"
                >
