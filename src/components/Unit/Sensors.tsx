      ...
      {isLargeScreen && (
        <div className="flex flex-row items-center gap-2">
          ...
          <button onClick={() => navigate(-1)}>
            <img src={back} className="h-6 w-6" alt="" />
          </button>
          <div className=""> Unit {unitId}</div>
        </div>
      )}

      <ul className="gap-[2rem] flex flex-col p-4">
        {unitData?.deviceList?.map((item: any) => {
          return (
            <li className="bg-[white] flex flex-col gap-1 p-2 rounded-lg ">
              <div
                className={`flex justify-between ${
                  isLargeScreen ? "flex-row" : "flex-col gap-2 "
                }`}
              >
                <div className="order-1">
                  <div className="flex flex-row gap-2">
                    {parseInt(item?.reading?.temperature?.split("°")[0]) >=
                      40 ||
                    parseInt(item?.reading?.humidity?.split("%")[0]) >= 40 ? (
                ...
                <div className="order-2 flex flex-row gap-1">
                  ...
                  <button className="w-[5.5rem] h-[2.4rem]  bg-[#00C17B] text-white rounded-md">
                    {item.connection}
                  </button>
                </div>
              </div>
              ...
              >
                ...
                >
                  {Object.keys(item.reading).map((key, idx) => (
                    <div
                      key={idx}
                      className="h-[2.4rem] flex flex-row gap-2 p-2 items-center justify-center h-[2.2rem] text-[red] rounded-md max-h-[2.4rem] w-[fit-content] w-[fit-content]"
                      style={{
                        backgroundColor:
                          key === "temperature"
                            ? "rgba(255, 87, 77, 0.24)"
                            : "#EDF1F7",
                        color: key === "temperature" ? "red" : "black",
                      }}
                    >
                      <img
                        className="h-6 w-6"
                        src={sensormapping[key] || sensormapping.default}
                        alt=""
                      />
                      <div>{key}</div>
